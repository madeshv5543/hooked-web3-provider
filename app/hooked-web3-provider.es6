  ...
  class HookedWeb3Provider extends Web3.providers.HttpProvider {
    ...
    send(payload, callback) {
      ...
      for (var request of requests) {
        if (request.method == "adh_sendTransaction") {
    ...
    // Rewrite all adh_sendTransaction payloads in the requests array.
    ...
    rewritePayloads(index, requests, session_nonces, finished) {
      ...
      if (payload.method != "adh_sendTransaction") {
        return next();
      }
      ...
      this.transaction_signer.hasAddress(sender, (err, has_address) => {
        ...
        var getNonce = (done) => {
          ...
          if (nonce != null) {
            ...
            this.sendAsync({
              jsonrpc: '2.0',
              method: 'adh_getTransactionCount',
        ...
        getNonce((err, nonce) => {
          ...
          this.transaction_signer.signTransaction(tx_params, function(err, raw_tx) {
            ...
            payload.method = "adh_sendRawTransaction";
...
if (typeof module !== 'undefined') {
  module.exports = factory(require("web3-adhi"));
} else {
